...
#define VIDEO_SOCK_IP "192.168.42.255"
#endif
...
#define VIDEO_DOWNSIZE_FACTOR 1
#endif

// From 0 to 99 (99=high)
#ifndef VIDEO_QUALITY_FACTOR
#define VIDEO_QUALITY_FACTOR 90
#endif
...
void *computervision_thread_main(void *data)
{
...
#else
  struct v4l2_device *dev = v4l2_init("/dev/video0", 320, 240, 60);
#endif
  if (dev == NULL) {
  ...
  if (opengl_init()) {
    printf("OpenGL succesfully started!\n");
  }
  ...
  uint8_t *imgbuf = (uint8_t *)malloc(dev->h * dev->w * 2);
  while (computer_vision_thread_command > 0) {
    ...
    printf("Really got image %d\n", img->idx);
    memcpy(imgbuf, img->buf, dev->h * dev->w * 2);


    // Save picture on disk
    if (computer_vision_thread_command == 2) {
      uint8_t *end = encode_image(imgbuf, jpegbuf, 99, FOUR_TWO_TWO, dev->w, dev->h, 1);
...
#else
      if (system("mkdir -p /data/ftp/internal_000/images") == 0) {
#endif
        ...
        for (; file_index < 99999; file_index++) {
...
#else
          sprintf(save_name, "/data/ftp/internal_000/images/img_%05d.jpg", file_index);
          sprintf(save_name2, "/data/ftp/internal_000/images/img_%05d.raw", file_index);
#endif
          ...
          if (access(save_name, F_OK) == -1) {
            ...
            save = fopen(save_name2, "w");
            if (save != NULL) {
              // Atomic copy
              viewvideo_save_shot_number = file_index;
              fwrite(img->buf, sizeof(uint8_t), dev->w * dev->h * 2, save);
              fclose(save);
            } else {
              printf("Error when opening file %s\n", save_name2);
            }
...
/////////////////////////////////////////////////////
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 320 / 2, 240, 0, GL_RGBA, GL_UNSIGNED_BYTE, imgbuf);
    ...
    glReadPixels(0, 0, 320 / 2, 240, GL_RGBA, GL_UNSIGNED_BYTE, imgbuf);
...
/////////////////////////////////////////////////////
    ...
    memset(jpegbuf, 0, dev->w * dev->h * 2);
    uint8_t *end = encode_image(imgbuf, jpegbuf, quality_factor, FOUR_TWO_TWO, dev->w, dev->h, dri_jpeg_header);
    uint32_t size = end - (jpegbuf);
